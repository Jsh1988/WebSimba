...
<head>
    ...
    <title><spring:message code="categories" /></title>
</head>
<body>
    <div id="block_body">
        <h1><spring:message code="welcome"/></h1>
            <div class="block_form_content">
                <div class="block_form">
                    <f3 class="form-f3-title"><spring:message code="categories" /></f3>
                        <c:url var="catDo" value="/categories.do" />
                        <form:form action="${catDo}" method="post" commandName="categories">
                            <ul id="block_log_pass">
                                ...
                                <li><a class="link" href="<c:url value="/categories?lang=en" />">en</a> || <a class="link" href="<c:url value="/categories?lang=ru" />">ru</a></li>
                            </ul>
                        </form:form>
                </div>
                        ...
                        <ul class="block_ul">
                            <h2><li><spring:message  code="id" /> || <spring:message code="title" /> || <spring:message code="alias" /> || <spring:message code="parent" /></li></h2>
                                <c:forEach items="${categoriesList}" var="categories">
                                    <li><c:out value="${categories.id}"/> || <c:out value="${categories.title}"/> || <c:out value="${categories.alias}"/> || <c:out value="${categories.parent}"/></li>
                                </c:forEach>
                        </ul>
                    </div>
                </div>
